...
def predict_endpoint():
    ...
    input_file, output_file = get_paths(ride['year'], ride['month'])

    df = read_data(input_file)
    df['ride_id'] = f'{ride["year"]:04d}/{ride["month"]:02d}_' + df.index.astype('str')
    categorical = ['PUlocationID', 'DOlocationID']
    ...
    result = {
        'mean_pred_duration': mean_pred_duration
    }
